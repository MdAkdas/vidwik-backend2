...
def get_video(id):
    try:
        video = SavedVideo.objects.get(id=int(id))
        ...
        data = {
            ...
            "gif": video.gif,
            "video_file":os.path.join(BASE_URL, video.video_file.url[1:]),
        ...
        for scene in all_scenes:
            ...
            if scene.video_file.name != "":
                data["scenes"][str(scene.order)]["uploaded_video"] = os.path.join(BASE_URL,
                                                                                  scene.uploaded_video.url[1:]),
        data["tags"] = []
